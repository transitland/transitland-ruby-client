describe TransitlandClient::Entities::OperatorInFeed do
  ...
  it 'can be created from the operator side' do
    ...
    operator_in_feed = TransitlandClient::Entities::OperatorInFeed.new(
      operator: operator,
  ...
  it 'fails gracefully when not enough arguments provided' do
    ...
    expect {
      TransitlandClient::Entities::OperatorInFeed.new(feed_onestop_id: 'f-9q9-BART')
    }.to raise_error(ArgumentError)

    expect {
      TransitlandClient::Entities::OperatorInFeed.new(operator_onestop_id: 'o-9q9-BART')
    }.to raise_error(ArgumentError)
  end
end
